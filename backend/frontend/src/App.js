...
class App extends Component {
    ...
    refreshList = () => {
        axios
            .get("http://127.0.0.1:9000/api/todos/")
    ...
    handleSubmit = item => {
        ...
        if (item.id) {
            axios
                .put(`http://127.0.0.1:9000/api/todos/${item.id}/`, item)
                .then(res => this.refreshList());
            return;
        }
        axios
            .post("http://127.0.0.1:9000/api/todos/", item)
            .then(res => this.refreshList());
    };
    handleDelete = item => {
        axios
            .delete(`http://127.0.0.1:9000/api/todos/${item.id}`)
            .then(res => this.refreshList());
    };
    createItem = () => {
